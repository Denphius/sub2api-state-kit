...
在「账号管理 → 编辑账号」里就能找到这个入口：选择 Pro / Team，单独开启 STATE，查看票据剩余时间和动态守护状态，也可以手动重新获取。
...
拿到代理连接信息后，在「系统设置 → 网关服务 → Codex 设置 → 全局动态 IP 池」填写完整代理 URL。程序支持自动更新代理用户名里的 `{sid}`，也适配了 1024proxy 的 SID 格式，不用每次手动生成、导入一堆 IP。
