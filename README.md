...
提供 **增量版本、完整部署版本、插件版本** 三种形式，**优先推荐增量版本**，方便在已有 Sub2API 源码上合并本扩展；完整部署版本和插件版本也可以按自己的部署方式选择，下载入口见下方。
...
| 形式 | 适合谁 | 下载与设置入口 |
| --- | --- | --- |
| **增量版（推荐）** | 已有源码，想查看或合并 STATE 修改 | [`overlay/` + `prepare.py`](#获取完整可构建源码)，基于 0.2.6；在账号编辑页配置 |
...
| **插件版（新增）** | 已在使用官方 Sub2API 0.2.7，希望保留原版宿主 | [插件版 v0.3.0](https://github.com/wangyunjeff/sub2api-state-kit/releases/tag/v0.3.0)，上传 `.s2plugin`，在插件配置页逐账号设置 |

**插件版不需要覆盖宿主源码。** 首次安装需要向宿主配置追加一次发布者公钥；支持 Linux amd64 / arm64，以及 macOS arm64。全局动态池、账号开关、Pro / Team、固定出口复验、续期与守护都在插件页操作。插件与源码版的入口、调度行为和 WebSocket 路径存在区别，详见 **[插件安装指南](docs/plugin.md)** 和 [插件验证记录](docs/plugin-validation.md)。同一个实例选择一种 STATE 实现即可，不建议叠加。
